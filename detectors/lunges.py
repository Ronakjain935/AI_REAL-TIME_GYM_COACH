...
class LungesDetector(BaseExercise):
    ...
    def process(self, landmarks) -> dict:
        ...
        key_landmarks_visible = (
            (getattr(landmarks[front_hip_idx], 'visibility', 1.0) or 0.0) >= self.MIN_VISIBILITY and
            (getattr(landmarks[front_knee_idx], 'visibility', 1.0) or 0.0) >= self.MIN_VISIBILITY and
            (getattr(landmarks[front_ankle_idx], 'visibility', 1.0) or 0.0) >= self.MIN_VISIBILITY
        )
